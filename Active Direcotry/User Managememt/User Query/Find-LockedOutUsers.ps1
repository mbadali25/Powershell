...
##
##  This script will go out there and find all Locked Out user accounts.
##
...
process {
    ...
    #Searches Active Directory for all Locked Out users accounts that aren't disabled
    $Results = (Search-ADAccount -Server $env:USERDNSDOMAIN -LockedOut -UsersOnly  -ResultPageSize 200 -resultSetSize $null | Where-Object { $_.enabled -eq $true }) | Where { $_.SamAccountName -ne "Administrator" }

    $subject = "Locked Out User Accounts Report: $(date)"
    if ($Results.length -eq 0) { $body = "There are no Locked Out Active Directory Users as of $($date)"; Write-Host "There are no Locked Out Active Directory Users as of $($date)" }
    if ($Results.length -gt 1) { 
        #Output Results
        Write-Host "There are $($Results.length) Locked Out Active Directory Users as of $($date)`n`n"
        $Results | ft

        $body = "There are $($Results.length) Locked Out Active Directory Users as of $($date)"
        ...
        $file = "$($Path)LockedOutUsers-$($date).csv"
