...
##
##  This script will go out there and find all Users with Expired Passwords
##
##  This is great when working with end users to see what Acitve Directory 
##  Accounts are Locked Out and unusabled.
##
...
process {
    ...
    #Searches Active Directory for all User accounts that have expired passwords and are enabled
    $Results = (Search-ADAccount -Server $env:USERDNSDOMAIN -PasswordExpired -UsersOnly  -ResultPageSize 200 -resultSetSize $null | Where-Object { $_.enabled -eq $true }) | Where { $_.SamAccountName -ne "Administrator" }

    $subject = "Locked Out User Accounts Report: $(date)"
    if ($Results.length -eq 0) { $body = "There are no Active Directory Users with Expired Passwords as of $($date)"; Write-Host "There are no Active Directory Users with Expired Passwords as of $($date)" }
    if ($Results.length -gt 1) { 
        #Output Results
        Write-Host "There are $($Results.length) Active Directory Users with Expired Passwords as of $($date)`n`n"
        $Results | ft

        $body = "There are $($Results.length) Active Directory Users with Expired Passwords as of $($date)"
        ...
        $file = "$($Path)ExpiredPasswordUsers-$($date).csv"
