...
param (
    ...
    HelpMessage="Name of the CSV Files with Users to add.")] 
    [string]$UserCSV
...
Process{    
    ...
    if($help)
    {
        ...
        '         The Add_to_AD_Group Fucntion will allow you to Add Users to the AD Group and logs the output

         The Below will just add a user to the specified AD Group.
         Exmaples: ./Add_to_AD_Group.ps1 -UserList "username" -ADGroupName "GroupName"

         If you need to add multiple users to the specified AD Group.
         Exmaples: ./Add_to_AD_Group.ps1 -UserList "username1,username2" -ADGroupName "GroupName"

         If you need to add CSV User List to add bulk users to the specified AD Group.
         Note: The csv first line "Header" must be username
         Exmaples: ./Add_to_AD_Group.ps1 -UserCSV "C:\filepath\usercsv.csv" -ADGroupName "GroupName"

         If you want speficiy a custom log path use the "-logpath" switch 
         Exmaples: ./Add_to_AD_Group.ps1 -UserList "username1","username2" -ADGroupName "GroupName" -logpath C:\somedirectory'
    ...
    else
    {
        ...
        Write-Host "`r`n$($ADGroupName) exist`r`n"
        ...
        foreach($user in $UserList)
        {
          if ( $PsCmdlet.ParameterSetName -eq "UserCSV")
          {
              ...
              { 
                ...
                #Checking if the usser is aleady a member of the group if not add them
                ...
                else { Write-Host"Adding User....`r`n"; Add-ADGroupMember -Identity $ADGroupNAme -Members $user.username -Verbose }
              }
              #If the Username is invalid it will output the invlaid username and continue
              catch
              {
                Write-Host "The Username is invalid. Unable to add $($user.username) to the Group $($ADGroupNAme)"
                Write-Host "The Current UserName $($user.username)"           
              }
           }
            ...
            else
            {
              ...
              {  
                ...
                else { Add-ADGroupMember -Identity $ADGroupName -Members $user -Verbose }
              }
              #If the Username is invalid it will output the invlaid username and continue
              catch
              {
                Write-Host "The Username is invalid. Unable to add $($user) to the Group $($ADGroupNAme)"
                Write-Host "The Current UserName $($user)"           
              }
            }
        }
